...
class Goods_received_noteForm(forms.ModelForm):
    ...
    issued = forms.IntegerField(label='Issued', initial="0")
...
class Issue_receipt_voucherForm(forms.ModelForm):
    ...
    date= forms.DateField(disabled=True,label='Date',
             initial=datetime.datetime.now())
    ...
    customer1=forms.CharField(label='Customer', max_length=300)
    date1= forms.DateField(widget=forms.TextInput(attrs={'readonly':'readonly'}),label='Date',
             initial=datetime.datetime.now())

    serial_number1 = forms.CharField(max_length=400, label='Serial Number')
    category1 = forms.CharField(label='category')
    subcategory1 = forms.CharField( max_length=400, label='subcategory')
    condition1 = forms.ChoiceField(choices=condition_choices, label='condition')

    quantity1 = forms.IntegerField(label='Quantity')
    description1 = forms.CharField(label='Description', max_length=300)
    order_no1 = forms.CharField(label='Order No.', max_length=200)
    stores_no1 = forms.CharField(label='Stores No', max_length=300)
    unit_cost1=forms.CharField(label='Unit Cost', max_length=300)

    comments1 = forms.CharField(widget=forms.Textarea(attrs={"rows":3, "cols":20}), max_length=1000, label='Comments')

    ordered_by1=forms.CharField(label='Ordered By', max_length=300)
    received_by1=forms.CharField(label='Received By', max_length=300)
    checked_by1 = forms.CharField(label='Checked by', max_length=300)
    class Meta:
